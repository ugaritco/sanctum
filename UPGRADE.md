...
Ugarit 11.0 is now the minimum required version.
...
```bash
php scribe vendor:publish --tag=sanctum-migrations
```
...
```php
'middleware' => [
    'authenticate_session' => Ugarit\Sanctum\Http\Middleware\AuthenticateSession::class,
    'encrypt_cookies' => Heritage\Cookie\Middleware\EncryptCookies::class,
    'validate_csrf_token' => Heritage\Foundation\Http\Middleware\ValidateCsrfToken::class,
],
```
